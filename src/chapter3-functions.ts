...
async function search3(query: string, tags?: string[]): Promise<Result[]> {
  ...
  return fetch(`https://example.com/api/search${queryString}`).then(res => res.json())
}
...
const testSearch: searchFn = async (term) => {
  return fetch(`https://example.com/api/search?query=${term}`).then(res => res.json())
}

// * Typescript uses something called substutibility
...
// * We can also use substutibility with function return types, but only for the return type void.
